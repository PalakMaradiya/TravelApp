...
import com.example.travelwithme.databinding.ActivityHillsDataBinding
import com.google.firebase.database.FirebaseDatabase
import com.google.firebase.storage.FirebaseStorage
import com.google.firebase.storage.StorageReference
import java.util.UUID

class HillsDataActivity : AppCompatActivity() {

    private val PICK_IMAGE_REQUEST = 100
    lateinit var uri: Uri
    lateinit var ImageUri: Uri

    lateinit var  binding: ActivityHillsDataBinding
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityHillsDataBinding.inflate(layoutInflater)
    ...
    private fun initView() {
        ...
        binding.btnSumbit.setOnClickListener {
            ...
            if (name.isEmpty()) {
                ...
                var key = reference.root.child("HillsTable").push().key ?: ""

                var modelclass = ModalClass(name, location, Description, price , key,ImageUri , rate, Day )

                reference.root.child("HillsTable").child(key).setValue(modelclass)
